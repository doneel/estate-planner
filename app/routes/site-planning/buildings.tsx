        ...
        <ul role="list" className="mt-3 grid grow grid-cols-1 gap-5 sm:grid-cols-1 sm:gap-6 lg:grid-cols-1">
          ...
          {buildingLibrary.length > 0 &&
            buildingLibrary.map((building) => (
              <li key={building.id} className="col-span-1 flex h-fit rounded-md shadow-sm">
                <div className={"flex w-16 flex-shrink-0 items-center justify-center rounded-l-md bg-gray-200 text-sm font-medium text-white"}>{building.initials}</div>
                <div className="flex flex-1 items-center justify-between truncate rounded-r-md border-t border-r border-b border-gray-200 bg-white">
                  <div className="flex-1 truncate px-4 py-2 text-sm">
                    <div className="flex font-medium text-gray-900 hover:text-gray-600">
                      {building.name ?? "Unnamed building"} <PencilSquareIcon className="ml-2 h-4 w-4 self-center hover:text-blue-400" />
                    </div>
